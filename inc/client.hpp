...
public:
    Client();
    Client(int cliFd, sockaddr_in& cliAdd, ServerConfig& server, int clientId);
    ~Client();
    ...
    bool            timeOut();
    int             getID() const { return _ClieId; }
    //
    ...
    void DeleteMethod(const std::string& path, Response& res);
};
