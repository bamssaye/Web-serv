...
void Server::initServer(){
    ...
    for (std::vector<std::pair<unsigned long, unsigned short> >::iterator it = _server.listen.begin();it != _server.listen.end(); ++it){
        ...
        std::cout << "Server listening on " << it->first << ":" << it->second << std::endl;
        }
...
void Server::_AcceptCon(int FdServer){
	...
	while (true){
		...
		// store client data;
        this->_CliCount++;
		_Clients[cliFd] = Client(cliFd, cliAdd, _server, this->_CliCount);
        this->_Msg("New client connected: Client id = " + _toString(_Clients[cliFd].getID()));
        // std::cout << "client         " << cliFd << std::endl;
	}
